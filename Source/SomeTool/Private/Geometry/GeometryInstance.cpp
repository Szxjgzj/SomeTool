				...
				for (int32 Index_Y = 0; Index_Y < Y; ++Index_Y)
				{
					for (int32 Index_X = 0; Index_X < X; ++Index_X)
					{
						...
						if (bIsUseRandomSize)
						{
							...
							const FRotator FinalRotator(
								ResultRotator.Pitch + Rotator_Delta.Pitch,
								ResultRotator.Yaw + Rotator_Delta.Yaw,
								ResultRotator.Roll + Rotator_Delta.Roll);
							InstanceTransform.SetLocation(Location);
...
TArray<FIntPoint> UGeometryInstance::GetPointsBy2dGrid(UShapeComponent* Shape, bool bIsAddInstance, float Distance)
{
	TArray<FIntPoint> FIntPoints;
	if (UBoxComponent* Box = Cast<UBoxComponent>(Shape))
	{
		if (Box->GetScaledBoxExtent().Z > 0)
		{
			return FIntPoints;
		}
		FVector Origin = Box->GetComponentLocation();
		...
		for (int32 Index_Y = 0; Index_Y < Y; ++Index_Y)
		{
			for (int32 Index_X = 0; Index_X < X; ++Index_X)
			{
				...
				InstanceTransform.SetLocation(Location);
				InstanceTransform.SetRotation(FRotator(0,0,0).Quaternion());
				InstanceTransform.SetScale3D(FVector::One());
						
				FIntPoints.Add(FIntPoint(Index_X, Index_Y));
				if (bIsAddInstance)
				{
					AddInstance(InstanceTransform,true);
				}
